...
public class AuthFilter implements Filter {
    ...
    private String getKeyForId(String id){
        ...
        Log.log(Level.INFO,"Map asking player service for key for id {0}",id);
        try {
    ...
    private boolean isDuplicate(String hmac) {
        if(requests.size() > TRIGGER_CLEANUP_DEPTH) {
            ...
            if(count > 0) {
                Log.log(Level.INFO,this,"Cleared down {0} hmacs",count);
            } else {
                Log.log(Level.INFO,this,"No messages were cleared down");
            }
        }
